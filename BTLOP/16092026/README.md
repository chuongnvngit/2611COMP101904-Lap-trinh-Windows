...
\### 2.1. Cấu trúc các lớp (Class \& Hierarchy)
...
&#x20;  - Các thuộc tính đóng gói: `MaNV`, `HoTen`, `LuongCoBan` (ràng buộc validation: lương cơ bản > 0).
...
&#x20;  - Thuộc tính bổ sung: `DoanhSo` (ràng buộc $\\ge 0$).
...
4\. \*\*`NhanVienThoiVu.cs` (Kế thừa từ `NhanVien`)\*\*:

&#x20;  - Thuộc tính bổ sung: `SoGioLam` ($\\ge 0$), `LuongTheoGio` ($> 0$).
...
&#x20; - Khởi tạo sẵn danh sách mẫu ít nhất 5 nhân viên thuộc cả 3 loại (Văn phòng, Kinh doanh, Thời vụ).
...
&#x20; - Tìm kiếm chính xác mã nhân viên (không phân biệt chữ hoa / chữ thường).
...
&#x20; - Thuật toán so sánh lương dựa trên việc gọi đa hình `nv.TinhLuong()`.
...
&#x20; - Tích lũy quỹ lương tự động qua `tongLuong += nv.TinhLuong();`, không phụ thuộc vào cấu trúc phân loại nhân viên.



\- \*\*Cơ chế chống Crash \& Xử lý ngoại lệ (Exception Handling)\*\*:

&#x20; - Sử dụng `int.TryParse` để chặn nhập chữ/ký tự lạ ở Menu mà không làm dừng đột ngột chương trình.

&#x20; - Xử lý các lựa chọn nằm ngoài khoảng (nhỏ hơn 0 hoặc lớn hơn 4).

&#x20; - Bọc toàn bộ chương trình trong khối `try - catch` để kiểm soát lỗi dữ liệu đầu vào.
...
1\. Mở file solution (`.sln`) trong thư mục bài tập bằng \*\*Visual Studio 2022\*\*.
